package thuypham.n16dccn159.ptithcm.sellingapp;

public class BR {
  public static final int _all = 0;

  public static final int cartCount = 1;

  public static final int category = 2;

  public static final int feeShip = 3;

  public static final int fragment = 4;

  public static final int listStatus = 5;

  public static final int order = 6;

  public static final int orderStatus = 7;

  public static final int price = 8;

  public static final int product = 9;

  public static final int totalPrice = 10;

  public static final int tranFee = 11;

  public static final int user = 12;

  public static final int userViewModel = 13;

  public static final int viewModel = 14;
}
