...
export const changePassword = (data, history) => async (dispatch: any) => {
    const api_change_pass = `/changePassword?userId=${data.userId}&oldPass=${data.oldPass}&newPass=${data.newPass}`;
    let res = await axios.get(api_change_pass);
    if ((res.data[0]).result === 1) {
        dispatch(getCurrentUser(data.userId));
        history.goBack();
    } else {
